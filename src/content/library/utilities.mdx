...
keywords: ["utility classes", "helpers", "spacing", "flex", "grid"]
npm: "@fylgja/utilities"
git: "https://github.com/fylgja/fylgja/tree/main/utilities"
sortOrder: 3
faq:
  - question: Do I need Fylgja Tokens installed for these utilities to work?
    answer: No. Fylgja Utilities work on their own; they don't require Fylgja Tokens
      or any other design token package to function.
  - question: Do I need a build step to use Fylgja Utilities?
    answer: No, these are production-ready CSS classes designed for direct use
      without a build step. You can link the compiled CSS directly or import
      only the utilities you need.
  - question: Will Fylgja Utilities conflict with Tailwind CSS or UnoCSS class names?
    answer: Generally no. Fylgja Utilities are designed to complement rather than
      replace other utility systems. If a class name overlaps with one you
      already use (e.g. flex), it's best to keep using whichever system already
      covers it and rely on Fylgja for the utilities that are unique to it.
  - question: How do breakpoint variants work for utilities like flow or spacing
      variables?
    answer: Most Dynamic CSS Utilities support a breakpoint prefix on their CSS
      variable, e.g. --md_my for margin-block at the md breakpoint, following
      the same pattern as --my. See Dynamic CSS Utilities for the full
      explanation.
  - question: What's the difference between the size utility and the spacing variables?
    answer: size provides class shorthands like w-full and h-full for common
      width/height cases. The spacing variables (--size-x, --size-y, --my, --mx,
      --py, --px) give you full control over width, height, margin, and padding
      without needing a dedicated class for every value.
  - question: Can I import a single utility instead of the whole package?
    answer: Yes, every utility listed in the overview can be imported individually
      with @import "@fylgja/utilities/{UTILITY_NAME}.css".
  - question: How do I set only one side with spacing variables like --my or --mx?
    answer: 'Pass two values to the logical shorthand; the first is the start side,
      the second the end. For example --mx: 0 1rem only spaces the inline-end,
      and --my: 1rem 0 only spaces the block-start. Set 0 (or your reset value)
      for the side you want to leave flush. These stay RTL-aware, so the start
      side follows the text direction.'
---
...
<InstallTabs pkg="@fylgja/utilities" npx="utilities" cdn="https://cdn.jsdelivr.net/npm/@fylgja/utilities/index.min.css" />
...
Fades the edges of a scrolling area, so it reads as "there is more this way"
instead of content stopping at a hard line. Pair it with `scroll-x`, or with
`scroll-y` plus the `scroll-y` class on the mask, which turns the fade vertical.

The fade follows the scroll position. At the very start only the end edge is faded,
in the middle both are, and at the very end only the start edge is. Nothing is
hidden when there is nothing more to scroll to. This runs on a scroll-driven
animation, so no JavaScript and no scroll listener.

<figure class="demo-wrapper">
<div class="scroll-x scroll-mask flex gap" style="--gap: 1rem">
<img src="/images/placeholders/cat.webp" alt="" width="480" height="320" loading="lazy" class="flex-none" style="--size-x: 240px" />
<img src="/images/placeholders/dog.webp" alt="" width="480" height="320" loading="lazy" class="flex-none" style="--size-x: 240px" />
<img src="/images/placeholders/bear.webp" alt="" width="480" height="320" loading="lazy" class="flex-none" style="--size-x: 240px" />
<img src="/images/placeholders/fox.webp" alt="" width="480" height="320" loading="lazy" class="flex-none" style="--size-x: 240px" />
</div>
</figure>

```html
<div class="scroll-x scroll-mask flex gap">
    <!-- Wide content that will scroll horizontally -->
</div>
```

| CSS Variable         | Description                                                          |
| -------------------- | -------------------------------------------------------------------- |
| `--mask-size`        | How far each edge fades, `1.5rem` by default                         |
| `--mask-dir`         | Gradient direction, set for you by the `scroll-y` class              |
| `--mask-color-start` | Opacity of the start edge, normally driven by the scroll position    |
| `--mask-color-end`   | Opacity of the end edge, normally driven by the scroll position      |

On an element that does not scroll, the scroll timeline never becomes active, so
both edges stay solid and you see no fade at all. To hold a fade on an element that
moves on its own, such as a marquee, set both color variables yourself:

```html
<div class="scroll-mask" style="--mask-color-start: transparent; --mask-color-end: transparent">
    <!-- Content that moves without scrolling -->
</div>
```

Browsers without scroll-driven animations show the plain mask with both edges
solid, so the content stays fully visible. The fade is an enhancement, never
...
Each of these maps to a logical shorthand (e.g. `--my` sets `margin-block`),
so you can target a single direction by passing two values:
the first is the start side, the second the end.

```html
<!-- margin only on the block-start (top in LTR) -->
<div style="--my: 1rem 0">...</div>
<!-- margin only on the inline-end (right in LTR, left in RTL) -->
<div style="--mx: 0 1rem">...</div>
```

Because these are logical properties, the start and end sides follow the text
direction, staying RTL-aware without any directional variants.
