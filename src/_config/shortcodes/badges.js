...
const badgeNpm = async (npmUrl) => {
    ...
    return `<a class="badge-addon -npm" href="${url}" target="_blank" rel="noopener noreferrer" title="${text}"><span class="badge -icon">${icon}</span><span class="badge">${version}</span></a>`;
};
...
const badgeGit = (
    ...
    const text = '<span class="aria-only">View code on </span>Github';
    ...
    return `<a href="${url}" target="_blank" rel="noopener noreferrer" title="View code on Github" class="badge-addon -git"><span class="badge -icon">${icon}</span><span class="badge">${text}</span></a>`;
};
